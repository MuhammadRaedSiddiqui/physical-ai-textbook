# Implementation Plan: Robot HUD Frontend Redesign

**Branch**: `001-robot-hud-redesign` | **Date**: 2025-12-16 | **Spec**: [spec.md](./spec.md)
**Input**: Feature specification from `/specs/001-robot-hud-redesign/spec.md`

## Summary

Redesign the Docusaurus frontend as a futuristic Robot Heads-Up Display (HUD) with animated grid backgrounds (GridScan), character decryption animations (DecryptedText), click spark effects (ClickSpark), card gallery (CardSwap), and spotlight cards (SpotlightCard). All components are React Bits components to be placed in `src/components/react-bits/`.

## Technical Context

**Language/Version**: TypeScript with React (Docusaurus v3)
**Primary Dependencies**: three.js, postprocessing, face-api.js, gsap, framer-motion, motion/react
**Storage**: N/A
**Testing**: Manual visual testing, Docusaurus build verification
**Target Platform**: Web (modern browsers, SSR-safe with BrowserOnly)
**Project Type**: Web application (Docusaurus frontend)
**Performance Goals**: Homepage loads < 3 seconds, 60fps animations
**Constraints**: SSR-safe (use BrowserOnly), respect prefers-reduced-motion
**Scale/Scope**: Single Docusaurus site with HUD aesthetic

## Constitution Check

*GATE: Must pass before Phase 0 research. Re-check after Phase 1 design.*

- [x] Single styling system (Tailwind CSS)
- [x] Components placed in designated location (`src/components/react-bits/`)
...
├── components/
│   └── react-bits/
│       ├── GridScan.tsx        # Animated CRT grid background
│       ├── DecryptedText.tsx   # Character decryption animation
│       ├── ClickSpark.tsx      # Click spark effect (replaces crosshair)
│       ├── CardSwap.tsx        # 3D card gallery with GSAP
│       └── SpotlightCard.tsx   # Spotlight hover effect card
├── css/
│   └── custom.css              # HUD theme styles
├── pages/
│   └── index.tsx               # Homepage with HUD components
└── theme/
    └── Root.tsx                # Animation providers wrapper
```

**Structure Decision**: Web application structure with React Bits components in dedicated folder.
...
**Purpose**: Animated CRT grid scanning background for hero section.
...
**Purpose**: Character-by-character "decryption" animation for headings.
...
### ClickSpark (replaces Crosshair)

**Purpose**: Click spark effect to replace the robotic crosshair cursor.
...
**Purpose**: 3D card gallery with GSAP animations for capstone project showcase.
...
1. Run `/sp.tasks` to generate the task list for implementation
2. Install required dependencies: `three`, `postprocessing`, `face-api.js`, `gsap`, `framer-motion`, `motion`
3. Create component files in `src/components/react-bits/`
4. Integrate components with Docusaurus using `BrowserOnly` wrapper for SSR safety
