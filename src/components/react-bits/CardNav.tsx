      ...
      >
        ...
        >
          {(items || []).slice(0, 3).map((item, idx) => (
            <div
              key={`${item.label}-${idx}`}
              className="nav-card select-none relative flex flex-col gap-2 p-[12px_16px] rounded-[calc(0.75rem-0.2rem)] min-w-0 flex-[1_1_auto] h-auto min-h-[60px] md:h-full md:min-h-0 md:flex-[1_1_0%] border border-opacity-30"
              ref={setCardRef(idx)}
              style={{ background: item.bgColor, color: item.textColor, borderColor: item.textColor }}
            >
              <div className="nav-card-label font-bold tracking-[-0.5px] text-[18px] md:text-[20px]" style={{ fontFamily: 'Inter, sans-serif', fontWeight: 700 }}>
                {item.label}
              </div>
